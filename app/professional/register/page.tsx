...
import { submitProfessionalRegistration, supabase } from '@/lib/supabase/client'
...
export default function ProfessionalRegisterPage() {
  ...
  // Fetch profession types from database
  useEffect(() => {
  ...
  const fetchProfessionTypes = async () => {
    try {
      const { data, error } = await supabase
        .from('v_profession_types_with_services')
        .select('*')
        .order('display_order')
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Find the selected profession type
      const selectedProfessionType = professionTypes.find(pt => pt.code === formData.profession)
      ...
      const professionalData = {
        ...
        profession_type_id: selectedProfessionType?.id, // Include profession_type_id
  ...
  // Dynamic profession options from database
  const professionOptions = professionTypes.map(pt => ({
    value: pt.code,
    label: { it: pt.name_it, en: pt.name_en },
    requiresLicense: pt.requires_license
  }))

  // Dynamic service options based on selected profession
  const currentProfessionType = professionTypes.find(pt => pt.code === formData.profession)
  const currentServices = currentProfessionType?.services.map(service => ({
    value: service.code,
    label: { it: service.name_it, en: service.name_en }
  })) || []
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  {currentProfessionType?.requires_license && (
                    <div>
                      <label className="block text-sm font-medium text-stone-700 mb-2">
